...
func NewDispatcher(workerNum int) *dispatcher {
	...
	for i := range workers {
		ch := make(chan *message.RecvMsg, 20)
...
func (d *dispatcher) OnClose(conn gnet.Conn, err error) (action gnet.Action) {
	...
	d.dispatch(&message.RecvMsg{Conn: conn, Logger: logger})
	return gnet.None
}
...
func (d *dispatcher) dispatch(msg *message.RecvMsg) {
	...
	msg.Logger.Debugf("dispatch %d packets (0 means close) to chan %d", len(msg.Pkts), workerIndex)
}
